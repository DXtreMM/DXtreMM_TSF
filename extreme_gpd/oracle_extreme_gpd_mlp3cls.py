...
class OracleExtremeGpd:
    ...
    def __init__(self, threshold, lag):
        ...
        # Right Extreme Model
        self.modelLExtreme = OracleExtremeGpd.getExtremeLModel(lag)
        # Left Extreme Model
        self.modelRExtreme = OracleExtremeGpd.getExtremeRModel(lag)
        
        # Normal Model
        self.modelNormal = OracleExtremeGpd.getNormalModel(lag)
    ...
    def predict(self, timeSeries, getAllOutputs=False):
        ...
        for i in range(predOutputs.shape[0]):
            if (timeSeries[i+self.lag]<=self.rthreshold and timeSeries[i+self.lag]>=self.lthreshold):
                predOutputs[i] = predNormal[i]
            elif timeSeries[i+self.lag]>self.rthreshold:
