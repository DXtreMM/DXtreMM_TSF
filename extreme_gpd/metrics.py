...
class Metric:
    ...
    @staticmethod
    def smape(forecastSeq: np.ndarray, actualSeq: np.ndarray):
        """
        ...
        of shape (n, d)
        :param actualSeq: Actual sequence, a numpy array of shape (n, d)
        :return: Returns SMAPE for each of the 'd' components as a numpy
        array of shape (d,)
        """
        ...
        return 100. * np.mean(
                np.abs((actualSeq - forecastSeq))
                / (np.abs(actualSeq) + np.abs(forecastSeq))
            , axis=0)
    ...
    def errorOnExtremes(
            ...
            extremelThreshold, extremerThreshold, func
    ):
        """
        ...
        maskArr = np.logical_or(actualSeq >= extremerThreshold, actualSeq <= extremelThreshold)
